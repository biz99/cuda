...
int main(int argc, char **argv)
{
	...
	start = clock();
	timedReduction<<<dimGrid, dimBlock, sizeof(float)*2*threads_per_blk>>>(dinput, doutput, dtimer);
	end = clock();

	cudaMemcpy(timer, dtimer, sizeof(clock_t) * number_of_blocks *2, cudaMemcpyDeviceToHost);
	const int number_of_return = 16384*2/(256*2);
	cudaMemcpy(result, doutput, sizeof(float)*number_of_return, cudaMemcpyDeviceToHost);
